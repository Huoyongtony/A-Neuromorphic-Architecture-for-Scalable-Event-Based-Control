...
class Graph:
    ...
    def __init__(self, syn_matrixs, syn_taus, syn_alphas, shifts, neuron_names,node_pos_by_name):
        ...
        self.color_index = 0
        self.node_pos_by_name = node_pos_by_name
    ...
    def _inject_cluster(self,html_path, clique_map, hub_colour):
        JS = f"""
        <script>
        ...
        function makeClusters(network){{
            Object.entries(cliqueMap).forEach(([centre, members]) => {{
                const nodes = [Number(centre), ...members];          // ★ centre IN cluster
                network.cluster({{
                    joinCondition : n => nodes.includes(n.id),
                    clusterNodeProperties : {{                       // ★ new id
                        id   : 'cluster_' + centre,
                        label: 'Centre ' + centre,
                        shape: 'dot',
                        size : 40,
                        color: '{hub_colour}',
                        borderWidth: 3,
                        allowSingleNodeCluster: true                // ★ robust for |clique| = 1
                    }}
                }});
            }});
        }}
        ...
        network.on('doubleClick', params => {{
            ...
            if (network.isCluster(id)) {{
                network.openCluster(id);            // expand big node
            }} else if (cliqueMap[id]) {{           // centre was double‑clicked
                makeClusters(network);              // re‑collapse its community
            }}
        }});
        </script>
        """
        ...
        with open(html_path, "r", encoding="utf-8") as f:
            html = f.read().replace("</body>", JS + "</body>", 1)
    ...
    def calculate_graph(self,matrix_find_centre):
        ...
        def _same_clique(u, v, list_of_centre):

            for i in range(len(list_of_centre)):
                if list_of_centre[i].get(u) == list_of_centre[i].get(v) and list_of_centre[i].get(u) is not None:
                    return True
        ...
        for i in range(len(matrix_find_centre)):
            if matrix_find_centre[i]==True:
                ...
                for member, centre in centre_of.items():
                    clique_members[centre].append(member)
        ...
        for rel_idx, M in enumerate(list_of_relation_matrix):
            colour, name, smooth, arrow_flag ,length = (edge_colours[rel_idx],
                                                relation_names[rel_idx],
                                                edge_smooth[rel_idx],
                                                edge_arrows[rel_idx],
                                                edge_lengths[rel_idx])
            n=M.shape[0]
            for j in range(n):          # source column
                for i in range(n):      # target row
                    ...
                    intra = _same_clique(i, j, list_of_centre)        # same clique?
                    # ---- 1B.  add the edge --------------------------------------
                    G.add_edge(
                        j, i,
                        title=f"{name}: {w}",
                        ...
                        arrows=arrows_spec,
                        length = 40 if intra else length,
                        physics = True
                    )
        ...
        for node in net.nodes:                 # node is a plain dict
        ...
        self._inject_cluster("four_relations_network.html",
                all_clique_members,
                HUB_COLOUR)
        
        self.all_clique_members = all_clique_members
        self.HUB_COLOUR = HUB_COLOUR
    ...
    def build_voltage_dashboard_cluster(
            self,
            volt_traces: np.ndarray,     # shape (N, T)
            video_file: str,             # e.g. "run_capture.mp4"
            out_dir: str = "animations",
            play_on_load: bool = False,
            fps: int = 30):
        """Build a dashboard that shows *one* panel with the recording video and
        *one* panel with the *four‑relation* graph created by
        :py:meth:`calculate_graph`. Nodes that correspond to biological neurons
        are recoloured frame‑by‑frame according to *volt_traces*.

        The graph **must** have been created beforehand (``self.net`` must not be
        ``None``).
        """
        ...
        # ---------------------------------------------------------------------
        if self.net is None:
            raise RuntimeError("Run calculate_graph() before build_voltage_dashboard().")

        N, T = volt_traces.shape
        if N < self.num_neuron:
            raise ValueError("volt_traces must have a row for every biological neuron.")

        Path(out_dir).mkdir(exist_ok=True)

        # ---------------------------------------------------------------------
        # 1.  save the full graph HTML and remember its filename
        # ---------------------------------------------------------------------
        graph_html = Path(out_dir) / "four_relations_network_anim.html"
        self.net.save_graph(str(graph_html))

     
        # ------------------------------------------------------------------
        # helper – inject an animation script into any PyVis page
        # ------------------------------------------------------------------
        ...
        # biological neuron ids are the integers < self.num_neuron
        node_ids = [n["id"] for n in self.net.nodes if isinstance(n["id"], int) and n["id"] < self.num_neuron]

        _inject_anim(graph_html, node_ids, volt_traces, volt_traces.min(), volt_traces.max())
        self._inject_cluster(graph_html, self.all_clique_members, self.HUB_COLOUR)

        # ---------------------------------------------------------------------
        # 3.  make a tiny video‑player HTML that emits the *frame* number
        # ---------------------------------------------------------------------
        ...
        function tick(now, metadata) {{
            parent.postMessage({{frame: Math.floor(metadata.mediaTime * FPS)}}, '*');
            vid.requestVideoFrameCallback(tick);
        }}

        vid.addEventListener('loadedmetadata', () => {{ vid.requestVideoFrameCallback(tick); }});
        vid.loop = false;
        {('vid.play();' if play_on_load else '')}
        </script></body></html>""", encoding="utf-8")

        # ---------------------------------------------------------------------
        # 4.  master dashboard (just two iframes + control buttons)
        # ---------------------------------------------------------------------
        dash_html = Path(out_dir) / "voltage_dashboard.html"
        dash_html.write_text(f"""<!doctype html><html><head><meta charset=\"utf-8\"><title>Dashboard</title>
        <style>
            ...
            <iframe class=\"panel\" src=\"{video_html.name}\"></iframe>
            <iframe class=\"panel\" src=\"{graph_html.name}\"></iframe>

            <script>
            const frames = [...document.querySelectorAll('iframe')].map(f => f.contentWindow);
            function go(cmd) {{ frames.forEach(f => f.postMessage(cmd, '*')); }}
            /* relay frame updates so both panels are always in sync */
            window.addEventListener('message', e => {{
                if (e.data && typeof e.data.frame === 'number')
                    frames.forEach(f => {{ if (f !== e.source) f.postMessage(e.data, '*'); }});
                if (e.data === 'reset')
